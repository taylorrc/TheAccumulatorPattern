...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        point1 = rg.Point(point.x + length, point.y + 30*k)
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        point2y = point.y -100 + (200*k/(n))
        #point2 = rg.Point(point.x + 100, point2y)
        left = rg.Point(point.x + 100*k, point2y)
        line = rg.Line(point, left)
